...
void Json::JsonArray::_JsonValueToStringHelper(Json::JsonValue* json_value, std::string& builder, int current_indent, int indent)
{
	...
	for (int i = 0; i < json_value->get_count(); i++)
	{
		...
		else
		{
			...
			arr->_JsonValueToStringHelper((*json_value)[i], builder, current_indent + indent, indent);
		}
...
void Json::JsonObject::changeKey(std::string old_key, std::string new_key)
{
	// Создаем новую пару с новым ключом и старым значением
	this->values[new_key] = this->values[old_key];

	// Удаляем старую пару
	this->values.erase(old_key);
}
...
Json::JsonValue* Json::JsonNumber::operator=(unsigned int value)
{
	this->value = value;
	return this;
}
...
Json::JsonValue* Json::JsonValue::operator=(unsigned int value)
{
	return nullptr;
}
...
void Json::JsonValue::changeKey(std::string old_key, std::string new_key)
{
}
