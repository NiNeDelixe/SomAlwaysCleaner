...
{
    // Получаем размер буфера, необходимого для хранения wchar_t, включая завершающий нулевой символ
    size_t buffer_size = 0;
    wchar_t* none = new wchar_t[str.size() + 1];;
    mbstowcs_s(&buffer_size, none, str.size() - 1, str.c_str(), str.size());

    //// Выделяем память под wchar_t* с использованием умного указателя
    //std::unique_ptr<wchar_t[]> wch_str(new wchar_t[str.size()]);

    //// Конвертируем строку в wchar_t
    //mbstowcs_s(,str.size(), wch_str.get(), str.c_str(), str.size());
...
std::wstring Additionals::Path::joinA(std::initializer_list<const char*> list)
{
    ...
    for (const char* elem : list)
    {
        ...
        mbstowcs_s(&out, wstr.get(), len, elem, len + 1);
